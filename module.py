# Helper function for requests with proper verification handling
def make_github_request(method, url, headers=None, json_data=None, is_source=False):
    """ Make a request to GitHub API with proper verification handling """
    verify = GH_SOURCE_VERIFY if is_source else GH_TARGET_VERIFY
    
    if method.lower() == 'get':
        return requests.get(url, headers=headers, verify=verify)
    elif method.lower() == 'post':
        return requests.post(url, headers=headers, json=json_data, verify=verify)
    elif method.lower() == 'patch':
        return requests.patch(url, headers=headers, json=json_data, verify=verify)
    elif method.lower() == 'delete':
        return requests.delete(url, headers=headers, verify=verify)
    else:
        raise ValueError(f"Unsupported HTTP method: {method}")

def write_migration_summary(output_folder, source_org, target_org, successful_migrations, failed_migrations, migration_details):
    """Write migration status to CSV file with detailed information
    Args:
        output_folder: Path to output folder
        source_org: Source organization name
        target_org: Target organization name
        successful_migrations: List of successfully migrated teams
        failed_migrations: List of failed team migrations
        migration_details: Dictionary containing detailed information about each team's migration
    """
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H%M")
    
    # Create summary file path with org names
    summary_file = os.path.join(output_folder, f"migration_summary_{source_org}_to_{target_org}_{timestamp}.csv")
    
    # Write summary with specified fields
    with open(summary_file, 'w', newline='') as csvfile:
        ...
        for team_name, details in migration_details.items():
            writer.writerow({
                'team_name': team_name,
                'team_status': 'success' if team_name in successful_migrations else 'failed',
                'total_members': details.get('total_members', 0),
                'members_migrated': details.get('members_migrated', 0),
                'members_failed': details.get('members_failed', 0),
                'idp_group_name': details.get('idp_group_name', ''),
                'idp_mapping_status': details.get('idp_mapping_status', 'not_attempted')
            })
...
def optimize_parent_child_migration(parent_child_relationships, team_cache):
    ...
    # Sort relationships to ensure parents are created before children
    dependency_graph = {}
    root_teams = []
    
    # Build dependency graph
    for child, parent in parent_child_relationships:
        if child not in dependency_graph:
            dependency_graph[child] = []
        dependency_graph[child].append(parent)
        
    # Find root teams (no parents)
    for team in team_cache.keys():
        if team not in dependency_graph:
            root_teams.append(team)
    
    # Build ordered execution plan (topological sort)
    execution_order = []
    visited = set()
    ...
    def visit(team):
        if team in visited:
            return
        visited.add(team)
        if team in dependency_graph:
            for parent in dependency_graph[team]:
                visit(parent)
        execution_order.append(team)
    
    # Visit all teams starting from roots
...
def get_all_external_groups(org_name, token):
    ...
    url = f"{GH_TARGET_BASE_API_URL}/orgs/{org_name}/external-groups"
    all_groups = []
    page = 1
    per_page = 100
    max_retries = 3
    ...
    while True:
        ...
        while retry_count <= max_retries:
            try:
                log_and_print(f"Fetching external IDP groups for organization '{org_name}' (page {page})")
                paginated_url = f"{url}?per_page={per_page}&page={page}"
                response = make_github_request('get', paginated_url, headers=headers)
                
                if response.status_code == 200:
                    groups_page = response.json().get('groups', [])
                    all_groups.extend(groups_page)
                    
                    # Check if we need to fetch more pages
                    if len(groups_page) < per_page:
                        # We got fewer items than requested per page, so we're done
                        return all_groups
                    else:
                        # Move to next page
                        page += 1
                    ...
                    log_and_print(f"Failed to fetch external IDP groups: {response.status_code} - {response.text}", "error")
                    return all_groups
                    
            except Exception as e:
                log_and_print(f"Error fetching external IDP groups: {str(e)}", "error")
        ...
        # If we've tried max_retries and still failing, return what we have so far
        if retry_count > max_retries:
            return all_groups
